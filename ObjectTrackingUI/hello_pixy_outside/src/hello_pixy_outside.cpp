...
int main(int argc, char * argv[])
{
  ...
  int		buf_size = 128;
  char     buf[buf_size];
 // ofstream pixy_data_file;
  //const char* general_log = "pixyGeneralLog.csv";
...
#endif
  printf("Opening data loging file...");
 // pixy_data_file.open(general_log);

  //if (pixy_data_file.is_open()) {
	  ...
	  while (run_flag)
	  {
		  ...
		  for (index = 0; index != blocks_copied; ++index) {
			 ...
			 // pixy_data_file.write(buf, buf_size);
			  printf("  %s\n", buf);
		  }
		  i++;
	  }
	 // pixy_data_file.close();
  /*}
  else {
